...
// 定义定时器预分频，定时器实际时钟频率为：sysClK /（GENERAL_TIMx_PRESCALER+1）
#define GENERAL_TIM_PRESCALER      (HAL_RCC_GetSysClockFreq()/1000000LU*10 - 1)  // 实际时钟频率为：100KHz

// 定义定时器周期，当定时器开始计数到GENERAL_TIMx_PERIOD值是更新定时器并生成对应事件和中断
#define GENERAL_TIM_PERIOD               50000u  // 定时器产生中断频率为：100KHz/50000=2Hz，即500ms定时周期
